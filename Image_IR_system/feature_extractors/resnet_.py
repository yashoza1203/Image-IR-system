...
class Resnet():
    def __init__(self) :
        ...
        with open(self.path+'/static/resnet50_features.pkl','rb') as f:  
            self.resnet_features=pkl.load(f)
        self.image_ids=list(self.resnet_features.keys())
        self.dir_path=self.path + '/static/dir.pkl'

    def evaluate_model(self,limit,diff_list,path,compare):
        path=path.name
        new_list=diff_list
    ...
    def get_sorted_list(self,query):
        ...
        qry_ftrs=self.resnet_features[img_id]
        qftrs=qry_ftrs / np.linalg.norm(qry_ftrs)
        
        for id in self.image_ids:
            ft=self.resnet_features[id] / np.linalg.norm(self.resnet_features[id])
